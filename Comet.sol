...
contract Comet is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 COMET-WETH LP -> COMET pool
    // uint256 public constant INITIAL_COMET_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the COMET ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("COMET", "COMET") {
        // Mints 1 COMET to contract creator for initial pool setup
    ...
    function _getCometPrice() internal view returns (uint256 _cometPrice) {
        try IOracle(cometOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Comet: failed to fetch COMET price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _cometPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_cometPrice >= taxTiersTwaps[tierId]) {
    ...
    function setCometOracle(address _cometOracle) public onlyOperatorOrTaxOffice {
        require(_cometOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints COMET to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of COMET to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentCometPrice = _getCometPrice();
            currentTaxRate = _updateTaxRate(currentCometPrice);
            if (currentCometPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        // _mint(_cometPool, INITIAL_COMET_POOL_DISTRIBUTION);
